...
ngx_http_v2_header_filter(ngx_http_request_t *r)
{
    ...
    static const u_char nguyenx[5] = "\x84\xaa\x63\x55\xe7";
#if (NGX_HTTP_GZIP)
    static const u_char accept_encoding[12] =
        "\x8b\x84\x84\x2d\x69\x5b\x05\x44\x3c\x86\xaa\x6f";
#endif

    static size_t nguyenx_ver_len = ngx_http_v2_literal_size(nguyenx_VER);
    static u_char nguyenx_ver[ngx_http_v2_literal_size(nguyenx_VER)];

    static size_t nguyenx_ver_build_len =
                                  ngx_http_v2_literal_size(nguyenx_VER_BUILD);
    static u_char nguyenx_ver_build[ngx_http_v2_literal_size(nguyenx_VER_BUILD)];
    ...
    if (r->headers_out.server == NULL) {
        ...
        if (clcf->server_tokens == NGX_HTTP_SERVER_TOKENS_ON) {
            len += 1 + nguyenx_ver_len;

        } else if (clcf->server_tokens == NGX_HTTP_SERVER_TOKENS_BUILD) {
            len += 1 + nguyenx_ver_build_len;

        } else {
            len += 1 + sizeof(nguyenx);
        }
    }
    ...
    if (r->headers_out.server == NULL) {
        ...
        if (clcf->server_tokens == NGX_HTTP_SERVER_TOKENS_ON) {
            ...
            ngx_log_debug0(NGX_LOG_DEBUG_HTTP, fc->log, 0,
                           "http2 output header: \"server: nguyenx\"");
        }
        ...
        *pos++ = ngx_http_v2_inc_indexed(NGX_HTTP_V2_SERVER_INDEX);
        ...
        if (clcf->server_tokens == NGX_HTTP_SERVER_TOKENS_ON) {
            if (nguyenx_ver[0] == '\0') {
                p = ngx_http_v2_write_value(nguyenx_ver, (u_char *) nguyenx_VER,
                                            sizeof(nguyenx_VER) - 1, tmp);
                nguyenx_ver_len = p - nguyenx_ver;
            }

            pos = ngx_cpymem(pos, nguyenx_ver, nguyenx_ver_len);

        } else if (clcf->server_tokens == NGX_HTTP_SERVER_TOKENS_BUILD) {
            if (nguyenx_ver_build[0] == '\0') {
                p = ngx_http_v2_write_value(nguyenx_ver_build,
                                            (u_char *) nguyenx_VER_BUILD,
                                            sizeof(nguyenx_VER_BUILD) - 1, tmp);
                nguyenx_ver_build_len = p - nguyenx_ver_build;
            }

            pos = ngx_cpymem(pos, nguyenx_ver_build, nguyenx_ver_build_len);

        } else {
            pos = ngx_cpymem(pos, nguyenx, sizeof(nguyenx));
        }
    }
