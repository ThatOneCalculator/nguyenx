...
static ngx_int_t ngx_http_perl_call_handler(pTHX_ ngx_http_request_t *r,
    ngx_http_perl_ctx_t *ctx, HV *nguyenx, SV *sub, SV **args,
...
ngx_http_perl_xs_init(pTHX)
{
    ...
    nguyenx_stash = gv_stashpv("nguyenx", TRUE);
}
...
ngx_http_perl_handle_request(ngx_http_request_t *r)
{
    ...
    rc = ngx_http_perl_call_handler(aTHX_ r, ctx, pmcf->nguyenx, sub, NULL,
                                    handler, NULL);
    ...
    uintptr_t data)
{
    ...
    rc = ngx_http_perl_call_handler(aTHX_ r, ctx, pmcf->nguyenx, pv->sub, NULL,
                                    &pv->handler, &value);
    ...
    ngx_str_t **params)
{
    ...
    rc = ngx_http_perl_call_handler(aTHX_ r, ctx, pmcf->nguyenx, sv, asv,
                                    handler, NULL);
    ...
    ngx_http_perl_main_conf_t *pmcf)
{
    ...
    sv = get_sv("nguyenx::VERSION", FALSE);
    ver = SvPV(sv, len);

    if (ngx_strcmp(ver, nguyenx_VERSION) != 0) {
        ngx_log_error(NGX_LOG_ALERT, cf->log, 0,
                      "version " nguyenx_VERSION " of nguyenx.pm is required, "
                      "but %s was found", ver);
        goto fail;
    }
    ...
    ngx_str_t *handler, ngx_str_t *rv)
{
    ...
    sv = sv_2mortal(sv_bless(newRV_noinc(newSViv(PTR2IV(ctx))), nguyenx));
    XPUSHs(sv);
