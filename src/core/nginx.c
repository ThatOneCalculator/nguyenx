...
{
    ngx_write_stderr("nguyenx version: " nguyenx_VER_BUILD NGX_LINEFEED);

    if (ngx_show_help) {
        ngx_write_stderr(
            "Usage: nguyenx [-?hvVtTq] [-s signal] [-p prefix]" NGX_LINEFEED
...
ngx_add_inherited_sockets(ngx_cycle_t *cycle)
{
    ...
    inherited = (u_char *) getenv(nguyenx_VAR);
        ...
        if (*p == ':' || *p == ';') {
            ...
            if (s == NGX_ERROR) {
                ngx_log_error(NGX_LOG_EMERG, cycle->log, 0,
                              "invalid socket number \"%s\" in " nguyenx_VAR
...
ngx_exec_new_binary(ngx_cycle_t *cycle, char *const *argv)
{
    ...
    var = ngx_alloc(sizeof(nguyenx_VAR)
    ...
    p = ngx_cpymem(var, nguyenx_VAR "=", sizeof(nguyenx_VAR));
